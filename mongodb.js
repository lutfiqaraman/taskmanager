...
MongoClient.connect(
  ...
  (error, client) => {
    ...
    // Update the user info - Users Database
    db.collection("users")
      .updateOne(
        {
          _id: new ObjectID("5e109db90d8bd75770779f77")
        },
        {
          $set: {
            name: "Mohammad Adnan",
    ...
    // Delete many users - Users Database
    db.collection("users")
      .deleteMany({
        age: 34
      })
    ...
    db.collection("tasks").findOne(
      { _id: new ObjectID("5e10adf9abf95c1f28250ccd") },
      (error, task) => {
        if (error) {
